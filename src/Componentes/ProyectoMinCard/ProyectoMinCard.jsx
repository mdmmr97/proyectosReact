...
const ProyectoMinCard = (props) => {
    ...
    function ObtenerNombre(alumno) {
        return alumno !== null ? <span className='card-text mb-0' key={alumno.id}>{alumno.nombre} {alumno.apellidos}</span> : <p></p>;
    }
    ...
    return (
        <div className="col-6 col-sm-4">
            <div className="card m-2 mt-3 rounded-5">
                ...
                <h5 className="card-title text-center mx-0 mb-0">{props.nombre}</h5>
                <div className="card-body m-0">
                    <p className='card-text alumnos'><span className="by-name">Alumnos:</span>{props.alumnos.map(ObtenerNombre)}</p>
                    <p className="card-text"><span className="by-name">Tutor:</span> {props.tutor}</p>
